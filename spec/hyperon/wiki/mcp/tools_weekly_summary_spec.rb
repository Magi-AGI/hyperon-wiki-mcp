...
RSpec.describe Hyperon::Wiki::Mcp::Tools, "weekly summary" do
  ...
  before do
    ...
    allow_any_instance_of(Hyperon::Wiki::Mcp::Client).to receive(:base_url).and_return(base_url)
    allow_any_instance_of(Hyperon::Wiki::Mcp::Auth).to receive(:token).and_return(valid_token)
    allow_any_instance_of(Hyperon::Wiki::Mcp::Auth).to receive(:username).and_return("TestUser")
  end
  ...
  describe "#format_weekly_summary" do
    ...
    it "includes repository changes section" do
      ...
      expect(result).to include("### hyperon-wiki")
      expect(result).to include("### hyperon-wiki-mcp")
